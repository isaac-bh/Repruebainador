            ...
            PythonShell.run('verificar.py', options, function (error, resultados) {
                ...
                // Si devuelve un resultado valido.
                    ...
                    if (resultados[0] == 0) {
                        let options = {
                            type: 'question',
                            buttons: ['Si porfavor', 'No gracias'],
                            defaultId: 0,
                            title: 'Archivo repetido',
                            message: '¿Quieres sobreescribir el archivo?',
                            detail: 'El nombre que ingresaste ya existe'
                        };

                        let des = dialog.showMessageBoxSync(null, options);
                        if (des == 0) {
                            loader.style.opacity = "1";
                            loader.style.display = "block";
                            ejecutar(ruta_carpeta);
                        } else {
                            loader.style.opacity = "0";
                            loader.style.display = "none";
...
function ejecutar(ruta_carpeta) {
    var id_examenes = document.getElementById("id_examenes").value;
    var nombre_examen = document.getElementById("nombre_examen").value;
